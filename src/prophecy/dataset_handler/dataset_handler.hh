...
class DatasetHandler
{
  public:
    ...
    std::vector<Tensor<dh_model_type>> binarize(void) const
    {
        ...
        {
            Tensor<dh_model_type> b_w(1, t.get_shape().at(1),
    t.get_shape().at(2));
    ...
    std::vector<Tensor<dh_model_type>>& get_training(void) { return x; }

    std::vector<Tensor<dh_model_type>>& get_labels(void) { return y; }

    long long get_limit(void) const { return limit_; }

    void set_limit(long long limit) { limit_ = limit; }
    ...
    void load_cifar_10(std::string path)
    {
        ...
        for (uint image = 0; image < nb_image; ++image)
        {
            ...
            unsigned char value;
            {
                ...
                value = buffer[offset];
                label(static_cast<uint>(value), 0u) =
                    static_cast<dh_model_type>(1);
                y.emplace_back(label);
            }

            Tensor<dh_model_type> rgb(
            ...
            for (uint channel = 0; channel < channel_size; ++channel)
            {
                for (uint y = 0; y < image_height; ++y)
                {
                    ...
                    {
                        value =
                            buffer[1 + offset + y * image_width +
                                   channel * (image_width * image_height) + x];
    ...
    void load_mnist_images(std::string path)
    {
        ...
        for (uint image = 0; image < nb_image; ++image)
        {
            ...
            for (uint y = 0; y < image_height; ++y)
            {
                ...
                {
                    const uint value =
                        static_cast<uint>(buffer[offset + y * image_width + x]);
