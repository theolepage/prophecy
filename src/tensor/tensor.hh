...
class Tensor
{
  public:
    ...
    std::vector<uint> get_shape() const { return *shape_; }
    ...
    Tensor<T> op(const Tensor& right, std::function<T(T, T)> fn) const
    {
        ...
        if (*shape_ != *right.shape_)
            throw std::invalid_argument(
                "Operations requires the two tensors to have the same shape.");
    ...
    Tensor<T>
    reduce(unsigned dim, T subtotal_default, std::function<T(T, T)> fn)
    {
    ...
    Tensor<T> im2col(const uint kernel_height,
                     ...
                     const uint stride)
    {
        ...
        const uint kernel_vertical_shifts =
            1 + (img_height + 2 * padding - kernel_height) / stride;
        const uint kernel_horizontal_shifts =
            1 + (img_width + 2 * padding - kernel_width) / stride;
        const uint patch = img_channels * kernel_height * kernel_width;

        Tensor<T> res(
            {patch, kernel_vertical_shifts * kernel_horizontal_shifts});
        ...
        for (uint c = 0; c < patch; c++)
        {
            ...
            for (uint i = 0; i < kernel_vertical_shifts; i++)
            {
                for (uint j = 0; j < kernel_horizontal_shifts; j++)
                {
                    ...
                    T value = 0;
                    if (y >= 0 && static_cast<uint>(y) < img_height && x >= 0 &&
                        static_cast<uint>(x) < img_width)
                        value = (*this)({channel,
                                         static_cast<uint>(y),
                                         static_cast<uint>(x)});
                ...
                for (uint j = 0; j < kernel_horizontal_shifts; j++)
                {
                    ...
                    if (y >= 0 && static_cast<uint>(y) < img_height && x >= 0 &&
                        ...
                        res({channel,
                             static_cast<uint>(y),
                             static_cast<uint>(x)}) +=
                            (*this)({c, i * kernel_horizontal_shifts + j});
                }
            }
        }
    ...
    Tensor<T> transpose() const
    {
        ...
        for (uint i = 0; i < rows; i++)
            for (uint j = 0; j < cols; j++)
                res.data_[offset_ + j * rows + i] =
                    data_[offset_ + i * cols + j];
