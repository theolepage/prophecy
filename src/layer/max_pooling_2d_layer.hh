...
class MaxPooling2DLayer final : public Layer<T>
{
  public:
    ...
    Tensor<T> feedforward(const Tensor<T>& input, bool training)
    {
        ...
        const uint out_rows =
            (height + 2 * this->padding_ - kernel_height) / this->stride_ + 1;
        const uint out_cols =
            (width + 2 * this->padding_ - kernel_width) / this->stride_ + 1;
        ...
        for (uint c = 0; c < channels; c++)
        {
            for (uint i = 0; i < out_rows; i++)
            {
                for (uint j = 0; j < out_cols; j++)
                {
                    ...
                    {
                        const int y = i * this->stride_ - this->padding_ +
                                      (k / kernel_width);
                        const int x = j * this->stride_ - this->padding_ +
                                      (k % kernel_width);

                        if (y < 0 || static_cast<uint>(y) >= height || x < 0 ||
                            static_cast<uint>(x) >= width)
                            continue;

                        T value = input(
                            {c, static_cast<uint>(y), static_cast<uint>(x)});
                        if (value >= maximum)
                        {
                            maximum        = value;
                            maximum_coords = Coord3D(
                                c, static_cast<uint>(y), static_cast<uint>(x));
                        }
                    }
                    out({c, i, j}) = maximum;
        ...
        for (uint c = 0; c < delta.get_shape()[0]; c++)
            for (uint i = 0; i < delta.get_shape()[1]; i++)
                for (uint j = 0; j < delta.get_shape()[2]; j++)
                    new_delta(this->mask_indices_->at(k++).to_list()) =
                        delta({c, i, j});
  ...
  private:
    class Coord3D
    {
      public:
        ...
        Coord3D(const uint x, const uint y, const uint z) : x_(x), y_(y), z_(z)
        {
        }

        std::vector<uint> to_list() { return {x_, y_, z_}; }
